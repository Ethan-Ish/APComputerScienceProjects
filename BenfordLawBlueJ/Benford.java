    ...
    public static double[] firstDigitFrequencies(String filename, int indexOfFirstDigit){
        ...
        try // try to read the file named file, if it exists
        {
            ...
            while (scanner.hasNextLine())
            {
                ...
                int num = Integer.parseInt(line.substring(indexOfFirstDigit, indexOfFirstDigit + 1));
                if(!(num == 0)){
                    tally[num] += 1;
                    totalNumbers += 1;
                }
        ...
        {
            double tally_Double = tally[i];
            double totalNum_Double = totalNumbers;
            double newVal = tally_Double / totalNum_Double;
            newVal *= 1000;
            int newVal_int = (int)Math.round(newVal);
            System.out.println(newVal_int);
            //newVal *= 0.1; //CURSE YOU FLOATING POINT ERRORS!
            String antiFloat = String.valueOf(newVal_int);
            antiFloat = antiFloat.substring(0,antiFloat.length() - 1) + "." + antiFloat.substring(antiFloat.length()-1);
            System.out.println(antiFloat);
            newVal = Double.parseDouble(antiFloat);
            frequencies[i-1] = newVal;
        }
